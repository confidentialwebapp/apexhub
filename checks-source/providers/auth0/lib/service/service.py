...
from apexhub.providers.auth0.exceptions.exceptions import (
    Auth0APIError,
    Auth0RateLimitError,
)
...
class Auth0Service:
    """Base class for Auth0 services: shared provider context and HTTP client."""
    ...
    def _get(self, path: str, params: dict = None) -> dict:
        """Make a rate-limit-aware GET request to the Auth0 API.

        Args:
            path: API path (e.g., "/api/v2/clients").
        ...
        Raises:
            Auth0RateLimitError: If rate limited after retries.
            Auth0APIError: If the API returns an error.
        """
        if params is None:
            params = {}
    ...
    def _paginate(self, path: str, key: str = None, params: dict = None) -> list:
        """Paginate through a Auth0 list endpoint.
