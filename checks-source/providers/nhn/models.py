...
class NHNOutputOptions(ProviderOutputOptions):
    ...
    def __init__(self, arguments, bulk_checks_metadata, identity: NHNIdentityInfo):
        ...
        if not getattr(arguments, "output_filename", None):
            # If tenant_id exists, include it in the filename (e.g., apexhub-output-nhn-<tenant_id>-20230101)
            if identity.tenant_id:
                self.output_filename = (
                    f"apexhub-output-nhn-{identity.tenant_id}-{output_file_timestamp}"
                )
            # Otherwise just 'apexhub-output-nhn-<timestamp>'
            else:
                self.output_filename = f"apexhub-output-nhn-{output_file_timestamp}"
