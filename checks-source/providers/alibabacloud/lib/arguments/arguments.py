def init_parser(self):
    ...
    alibabacloud_auth_subparser.add_argument(
        ...
        nargs="?",
        default=None,
        help="ARN of the RAM role to assume (e.g., acs:ram::123456789012:role/APEX HubAuditRole). Requires access keys to be set via environment variables (ALIBABA_CLOUD_ACCESS_KEY_ID and ALIBABA_CLOUD_ACCESS_KEY_SECRET). The provider will automatically obtain and refresh STS tokens. Can also use ALIBABA_CLOUD_ROLE_ARN environment variable",
    )
    alibabacloud_auth_subparser.add_argument(
        "--role-session-name",
        nargs="?",
        default=None,
        help="Session name when assuming the RAM role. Defaults to APEX HubAssessmentSession. Can also use ALIBABA_CLOUD_ROLE_SESSION_NAME environment variable",
    )
    alibabacloud_auth_subparser.add_argument(
        "--ecs-ram-role",
        nargs="?",
    ...
    alibabacloud_regions_subparser.add_argument(
        ...
        nargs="+",
        dest="regions",
        help="Alibaba Cloud region IDs to run APEX Hub against (e.g., cn-hangzhou, cn-shanghai)",
    )
