...
from apexhub.providers.anthropic.exceptions.exceptions import (
    AnthropicAPIError,
...
class AnthropicService:
    """Base class for Anthropic Console services: shared provider context and HTTP client."""
    ...
    def __init__(self, service: str, provider):
        ...
        self._http_session.headers.update(
            {
                "x-api-key": provider.session.token,
    ...
    def _get(self, path: str, params: dict = None) -> dict:
        """Make a rate-limit-aware GET request to the Anthropic Console API.

        Args:
            path: API path (e.g., "/v1/organizations/workspaces").
        ...
        Raises:
            AnthropicRateLimitError: If rate limited after retries.
            AnthropicAPIError: If the API returns an error.
        """
        if params is None:
            params = {}
    ...
    def _paginate(self, path: str, key: str = None, params: dict = None) -> list:
        """Paginate through a Anthropic Console list endpoint.
