...
from apexhub.providers.vault.exceptions.exceptions import (
    VaultAPIError,
    VaultRateLimitError,
)
...
class VaultService:
    """Base class for HashiCorp Vault services: shared provider context and HTTP client."""
    ...
    def __init__(self, service: str, provider):
        ...
        self._http_session.headers.update(
            {
                "X-Vault-Token": provider.session.token,
    ...
    def _get(self, path: str, params: dict = None) -> dict:
        """Make a rate-limit-aware GET request to the HashiCorp Vault API.

        Args:
            path: API path (e.g., "/v1/sys/seal-status").
        ...
        Raises:
            VaultRateLimitError: If rate limited after retries.
            VaultAPIError: If the API returns an error.
        """
        if params is None:
            params = {}
    ...
    def _paginate(self, path: str, key: str = None, params: dict = None) -> list:
        """Paginate through a HashiCorp Vault list endpoint.
