...
from apexhub.providers.jenkins.exceptions.exceptions import (
    JenkinsAPIError,
...
class JenkinsService:
    """Base class for Jenkins services: shared provider context and HTTP client."""
    ...
    def __init__(self, service: str, provider):
        ...
        self._http_session.headers.update(
            {
                "Authorization": f"Basic {provider.session.token}",
    ...
    def _get(self, path: str, params: dict = None) -> dict:
        """Make a rate-limit-aware GET request to the Jenkins API.

        Args:
            path: API path (e.g., "/api/json").
        ...
        Raises:
            JenkinsRateLimitError: If rate limited after retries.
            JenkinsAPIError: If the API returns an error.
        """
        if params is None:
            params = {}
    ...
    def _paginate(self, path: str, key: str = None, params: dict = None) -> list:
        """Paginate through a Jenkins list endpoint.
