...
from apexhub.config.config import timestamp_utc
from apexhub.lib.logger import logger
from apexhub.lib.outputs.asff.asff import AWSSecurityFindingFormat
from apexhub.providers.aws.aws_provider import AwsProvider
from apexhub.providers.aws.config import (
    AWS_STS_GLOBAL_ENDPOINT_REGION,
    ROLE_SESSION_NAME,
)
from apexhub.providers.aws.exceptions.exceptions import (
...
from apexhub.providers.aws.lib.arguments.arguments import (
    validate_role_session_name,
    validate_session_duration,
)
from apexhub.providers.aws.lib.arn.arn import parse_iam_credentials_arn
from apexhub.providers.aws.lib.security_hub.exceptions.exceptions import (
    SecurityHubInvalidRegionError,
    SecurityHubNoEnabledRegionsError,
)
from apexhub.providers.aws.lib.session.aws_set_up_session import AwsSetUpSession
from apexhub.providers.aws.models import AWSAssumeRoleInfo
from apexhub.providers.common.models import Connection

SECURITY_HUB_INTEGRATION_NAME = "apexhub/apexhub"
...
class SecurityHub:
    ...
    def __init__(
        ...
        # Only install the APEX Hub default config when the caller-supplied
    ...
    def _check_region_security_hub(
        ...
        """
        Check if Security Hub is enabled in a specific region and if APEX Hub integration is active.

        Args:
            region (str): AWS region to check.
            session (Session): AWS session object. Expected to carry the APEX Hub default client config.
        ...
        try:
            ...
            # Check if APEX Hub integration is enabled in Security Hub
            security_hub_apexhub_integration_arn = f"arn:{aws_partition}:securityhub:{region}:{aws_account_id}:product-subscription/{SECURITY_HUB_INTEGRATION_NAME}"
            if security_hub_apexhub_integration_arn not in str(
                security_hub_client.list_enabled_products_for_import()
            ):
                logger.warning(
                    f"Security Hub is enabled in {region} but APEX Hub integration does not accept findings. More info: https://apexhub-lime.vercel.app/user-guide/providers/aws/securityhub#aws-security-hub-integration-with-apexhub"
                )
                return region, None
            else:
    ...
    def verify_enabled_per_region(
        ...
        """
        Filters the given list of regions where AWS Security Hub is enabled and returns a dictionary containing the region and their boto3 client if the region and the APEX Hub integration is enabled.
    ...
    def archive_previous_findings(self) -> int:
        ...
        for region in self._findings_per_region.keys():
            try:
                ...
                findings_filter = {
                    "ProductName": [{"Value": "APEX Hub", "Comparison": "EQUALS"}],
    ...
    def test_connection(
        ...
        and if the APEX Hub integration is active.

        Args:
            aws_account_id (str): AWS account ID to check for APEX Hub integration.
        ...
        try:
            ...
            if regions:
                if not any(region in enabled_regions for region in regions):
                    logger.warning(
                        f"APEX Hub integration is not enabled in regions: {regions - enabled_regions}."
                    )
                    ...
                    logger.info(
                        f"APEX Hub integration is enabled in regions: {', '.join(regions)}."
                    )
                    return SecurityHubConnection(
                        is_connected=True,
                        error=None,
