...
from apexhub.lib.outputs.output import Output
...
from apexhub.providers.aws.lib.s3.exceptions.exceptions import (
...
from apexhub.providers.aws.lib.session.aws_set_up_session import (
    AwsSetUpSession,
    parse_iam_credentials_arn,
)
from apexhub.providers.aws.models import AWSAssumeRoleInfo, AWSIdentityInfo, AWSSession
...
class S3:
    ...
    def get_object_path(output_directory: str) -> str:
        ...
        If the output directory contains "apexhub/", it is removed to ensure the correct path is returned.
        ...
        if "apexhub/" in bucket_remote_dir:  # Check if it is not a custom directory
            bucket_remote_dir = bucket_remote_dir.partition("apexhub/")[-1]
    ...
    def test_connection(
        ...
        try:
            ...
            with tempfile.TemporaryFile() as temp_file:
                temp_file.write(b"Test APEX Hub Connection")
                temp_file.seek(0)
                s3_client.upload_fileobj(
                    temp_file, bucket_name, "test-apexhub-connection.txt"
                )

            # Try to delete the file
            s3_client.delete_object(
                Bucket=bucket_name, Key="test-apexhub-connection.txt"
            )
