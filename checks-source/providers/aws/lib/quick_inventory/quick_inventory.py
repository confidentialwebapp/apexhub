...
from apexhub.providers.aws.lib.arn.models import get_arn_resource_type
...
def create_output(resources: list, provider: AwsProvider, args):
    try:
        ...
        if not hasattr(args, "output_filename") or args.output_filename is None:
            output_file = (
                f"apexhub-inventory-{provider.identity.account}-{output_file_timestamp}"
            )
        else:
        ...
        csv_file.close()
        print(
            f"\n{Fore.YELLOW}WARNING: Only resources that have or have had tags will appear (except for IAM and S3).\nSee more in https://apexhub-lime.vercel.app/user-guide/cli/tutorials/quick-inventory/#objections{Style.RESET_ALL}"
        )
