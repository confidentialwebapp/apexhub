...
from apexhub.providers.aws.config import ROLE_SESSION_NAME
from apexhub.providers.aws.lib.arn.arn import arn_type
...
def init_parser(self):
    ...
    aws_auth_subparser.add_argument(
        ...
        nargs="?",
        default=None,
        help="AWS profile to launch apexhub with",
    )
    aws_auth_subparser.add_argument(
        "--role",
        "-R",
    ...
    aws_auth_subparser.add_argument(
        ...
        help="An identifier for the assumed role session. Defaults to APEX HubAssessmentSession",
    ...
    aws_regions_subparser.add_argument(
        ...
        "-f",
        nargs="+",
        help="AWS region names to run APEX Hub against",
    ...
    aws_regions_subparser.add_argument(
        ...
        help=(
            ...
            "APEXHUB_AWS_DISALLOWED_REGIONS environment variable when set."
        ),
        choices=AwsProvider.get_regions(partition=None),
    )
    # AWS Organizations
    ...
    aws_security_hub_subparser.add_argument(
        "--skip-sh-update",
        action="store_true",
        help="Skip updating previous findings of APEX Hub in Security Hub",
    )
    aws_security_hub_subparser.add_argument(
        "--send-sh-only-fails",
        action="store_true",
        help="Send only APEX Hub failed findings to SecurityHub",
    )
    # AWS Quick Inventory
    ...
    aws_quick_inventory_subparser.add_argument(
        ...
        "-i",
        action="store_true",
        help="Run APEX Hub Quick Inventory. The inventory will be stored in an output csv by default",
    )
    # AWS Outputs
    ...
    aws_based_scans_parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Scan only resources with specific AWS Tags (Key=Value), e.g., Environment=dev Project=apexhub",
    )
    aws_based_scans_parser.add_argument(
        "--resource-arn",
        "--resource-arns",
    ...
    # APEX Hub Fixer
    apexhub_fixer_subparser = aws_parser.add_argument_group("APEX Hub Fixer")
    apexhub_fixer_subparser.add_argument(
        "--fixer",
        action="store_true",
        help="Fix the failed findings that can be fixed by APEX Hub",
    )
