...
from apexhub.config.config import (
    aws_services_json_file,
    default_config_file_path,
    get_default_mute_file_path,
    load_and_validate_config_file,
)
from apexhub.lib.check.utils import list_modules, recover_checks_from_service
from apexhub.lib.logger import logger
from apexhub.lib.utils.utils import open_file, parse_json_file, print_boxes
...
from apexhub.providers.aws.lib.arn.models import ARN
from apexhub.providers.aws.lib.mutelist.mutelist import AWSMutelist
from apexhub.providers.aws.lib.organizations.organizations import (
    get_organizations_metadata,
    parse_organizations_metadata,
)
from apexhub.providers.aws.models import (
...
from apexhub.providers.common.models import Audit_Metadata, Connection
...
class AwsProvider(Provider):
    ...
    def __init__(
        ...
        Args:
            ...
            - excluded_regions: A set of regions to skip during the scan. Applied
              ...
              settable via the APEXHUB_AWS_DISALLOWED_REGIONS environment variable
        ...
        Usage:
            - Boto3 is used so we follow their credential setup process:
                ...
                - Assume Role: *Requires authentication.* APEX Hub can be used against multiple accounts using IAM Assume Role features depending on each use case:
    ...
    def setup_session(
        ...
        Args:
            ...
            - session_config: Botocore Config applied as the session's default
              ...
              the APEX Hub user agent and retry settings.
    ...
    def print_credentials(self):
        ...
        Caller Identity ARN: arn:aws:iam::123456789012:user/apexhub
        ```
        """
            ...
            for resource in self._audit_resources:
                ...
                if service != "wafv2" and service != "waf":
                    ...
                    if service == "lambda":
                        ...
                        service = "cloudwatch"
                    # Check if APEX Hub has checks in service
                    try:
    ...
    def set_session_config(retries_max_attempts: int) -> Config:
        """
        set_session_config returns a botocore Config object with the APEX Hub user agent and the default retrier configuration if nothing is passed as argument
    ...
    def test_connection(
        ...
        Examples:
            >>> AwsProvider.test_connection(
                role_arn="arn:aws:iam::111122223333:role/APEX HubRole",
