...
from apexhub.providers.aws.services.cloudtrail.cloudtrail_client import (
    cloudtrail_client,
)
from apexhub.providers.aws.services.s3.s3_client import s3_client
        ...
        if cloudtrail_client.trails is not None:
            for trail in cloudtrail_client.trails.values():
                if trail.name:
                    ...
                    # check if trail is delivering logs in a cross account bucket or another region out of APEX Hub's audit scope
                    if not trail_bucket_is_in_account:
                        report.status = "MANUAL"
                        report.status_extended = f"Trail {trail.name} is delivering logs to bucket {trail_bucket} which is a cross-account bucket or out of APEX Hub's audit scope, please check it manually."
