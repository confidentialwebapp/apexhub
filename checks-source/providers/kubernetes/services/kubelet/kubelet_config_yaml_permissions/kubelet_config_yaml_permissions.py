from apexhub.lib.check.models import Check, Check_Report_Kubernetes
from apexhub.lib.utils.utils import get_file_permissions
from apexhub.providers.kubernetes.services.core.core_client import core_client
...
class kubelet_config_yaml_permissions(Check):
    def execute(self) -> Check_Report_Kubernetes:
        ...
        for node in core_client.nodes.values():
            ...
            # It can only be checked if APEX Hub is being executed inside a worker node or if the file is the default one
            if node.inside:
                ...
                report.status = "MANUAL"
                report.status_extended = f"APEX Hub is not being executed inside Node {node.name}, please verify kubelet config.yaml file permissions manually."
