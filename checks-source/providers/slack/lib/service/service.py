...
from apexhub.providers.slack.exceptions.exceptions import (
    SlackAPIError,
    SlackRateLimitError,
)
...
class SlackService:
    """Base class for Slack services: shared provider context and HTTP client."""
    ...
    def _get(self, path: str, params: dict = None) -> dict:
        """Make a rate-limit-aware GET request to the Slack API.

        Args:
            path: API path (e.g., "/api/team.info").
        ...
        Raises:
            SlackRateLimitError: If rate limited after retries.
    ...
    def _paginate(self, path: str, key: str = None, params: dict = None) -> list:
        """Paginate through a Slack list endpoint.
