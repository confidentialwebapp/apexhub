...
from apexhub.providers.terraformcloud.exceptions.exceptions import (
    TerraformCloudAPIError,
    TerraformCloudRateLimitError,
)
...
class TerraformCloudService:
    """Base class for HCP Terraform services: shared provider context and HTTP client."""
    ...
    def _get(self, path: str, params: dict = None) -> dict:
        """Make a rate-limit-aware GET request to the HCP Terraform API.

        Args:
            path: API path (e.g., "/api/v2/organizations").
        ...
        Raises:
            TerraformCloudRateLimitError: If rate limited after retries.
            TerraformCloudAPIError: If the API returns an error.
        """
        if params is None:
            params = {}
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if response.status_code == 429:
                    ...
                    if attempt < max_retries:
                        ...
                        continue
                    raise TerraformCloudRateLimitError(
                        file=__file__,
            ...
            except TerraformCloudRateLimitError:
                raise
            except requests.exceptions.HTTPError as error:
                raise TerraformCloudAPIError(
                    file=__file__,
    ...
    def _paginate(self, path: str, key: str = None, params: dict = None) -> list:
        """Paginate through a HCP Terraform list endpoint.
